...
O token é gerado após o login do usuário e deve ser enviado em todas as requisições protegidas por meio do header
`Authorization`.
...
Algumas rotas são protegidas por autenticação JWT e podem exigir permissões específicas de acordo com o papel do
usuário.
...
- **Parâmetros de query:**
    - name (opcional) - Termo utilizado para filtrar grupos musculares pelo nome.
      A busca retorna todos os registros que contenham o termo informado, de forma parcial e sem distinção entre
